...
void Shell_configure(Shell *self, int argc, char *argv) {
  ...
  ArgsHandler_insert_arguments(args_handler, Arg_init("-o", "--out-file", STRING(NULL), self->config, 1, ShellConfig_set_out_file));
  /* set the Logger error file */
  ArgsHandler_insert_arguments(args_handler, Arg_init("-e", "--err-file", STRING(NULL), self->config, 1, ShellConfig_set_err_file));
  /* set if the Logger have to print the return code of the processes*/
  ArgsHandler_insert_arguments(args_handler, Arg_init("-c", "--code", STRING(NULL), self->config, 0, ShellConfig_enable_print_code));
  /* set the max size in characters of the output of every process */
  ArgsHandler_insert_arguments(args_handler, Arg_init("-m", "--max-size", STRING(NULL), self->config, 1, ShellConfig_set_output_max_size));
  /* set the logger process executable path 
  */
  ArgsHandler_insert_arguments(args_handler, Arg_init("-l", "--logger-path", STRING(NULL), self->config, 1, ShellConfig_set_logger_path));
